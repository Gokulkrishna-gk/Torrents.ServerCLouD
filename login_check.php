...
if (isset($_COOKIE['torrents_svrcd_session']) && trim($_COOKIE['torrents_svrcd_session']) != '') { // user has a session already?
	...
	if ($check_flood_control->num_rows > 0) {
		...
		if ($flood_control_info['tsc'] < (time() - 1800)) {
			...
			if ($flood_control_info['attempts'] > 20) {
				...
				die("Ouch, you are flooding us with too many login request,Please take a rest and continue your attempt after half an hour or so.");
			}
		}
	}
	...
	if ($check_for_user->num_rows == 1) {
		...
		if (crypt(trim($_POST['p']), $current_user_row['pwrdlol']) != $current_user_row['pwrdlol']) {
			...
			die('Ehh? Password incorrect? Please double check it ya~');
		} else {
		...
		die('Ouch, Error 404 Email not found. Please take a look at our registration corner~');
	}
